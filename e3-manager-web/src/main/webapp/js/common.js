...
//    	后台使用单页面处理，".selectItemCat"所表示的按钮有多个，所以在此使用循环进行处理
    	$(".selectItemCat").each(function(i,e){
//    		参数i表示循环的索引下标，e表示原生的js dom对象
    		var _ele = $(e);//原生的js dom对象转化成jquery对象
    		if(data && data.cid){
...
//    			打开一个窗口
    			.window({
    				width:'500',
    			    height:"450",
    			    modal:true,//这个属性设定了一旦弹框出现之后，其后边的北背景就会点击不了
    			    closed:true,
    			    iconCls:'icon-save',
    			    title:'选择类目',
//    			    表示一旦打开了这个窗口之后就会调用的事件
    			    onOpen : function(){
    			    	var _win = this;
    			    	$("ul",_win).tree({
    			    		url:'/item/cat/list',
